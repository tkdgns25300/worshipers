...
- **집회 상세(`gatherings/[id]`)**: **보딩패스 "입장권"** — 상태 점 + 천공 정보행(일시+D-day · 장소+카카오/네이버 길찾기 · 입장 · 등록 · 온라인) + 케이스별 CTA(등록하기/다시보기/공식공지 + 공유 보조). **라벨형 참석 안내**(note의 `라벨 — 내용` 줄) · 정보 출처 · 주최 팀 + "다른 모임"(반복 제외 시리즈 수, 0이면 숨김). **집회별 OG 이미지**(`opengraph-image.tsx`, 카카오 키 불필요).
