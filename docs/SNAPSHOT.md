...
- **단계**: Phase 1 — **홈 · 집회 상세 · 팀 디렉토리 재설계 완료 + 광고 제거 + home-view 분해**. 핵심 화면 디자인은 일단락. 다음 = 데이터 검수 / 팀 상세 카드 통일 / 배포.
...
1. **팀 상세(`teams/[id]`) 카드 통일** — 아직 `GatheringCard`(캘린더 타일). 홈/디렉토리 톤(티켓)과 맞출지 결정 후 적용.
2. **데이터 전수 검수(ROADMAP 1-9)** — `TODO(검수)`/`TODO(운영)` 일괄 확정(sourceUrl·마커스 온라인·표기 통일·주간 날짜 연장). + 남은 6팀(기프티드·브리지임팩트·히즈윌·아가파오워십·웨이홈·키퍼스워십) team-only 등록.
3. **배포** — Vercel + 도메인 `worshipers.life` + GA4/Search Console. ⚠️ 공개 전 **팀 로고 허가** 확보.
4. **(선택·보류)** 카카오톡 공유(Kakao JS키 필요) · 카드 반복 배지.
...
- 남은 팀 자료 + 각 팀 집회 공지 · 토스 송금 링크/QR · 팀 로고 사용 허가 · (선택) 카카오 JS키
